---
description: Pelajari cara menggunakan RcloneView dengan Wasabi untuk menjelajahi, mencadangkan, sinkronisasi, dan migrasi data antara penyimpanan lokal dan cloud lainnya.
keywords:
  - rcloneview
  - wasabi
  - s3-compatible
...
# Bagaimana cara menggunakan RcloneView dengan Wasabi?

RcloneView adalah aplikasi desktop yang memberikan Anda Explorer dua panel yang visual untuk **rclone**. Aplikasi ini memungkinkan Anda menyalin, sinkronisasi, dan migrasi berkas antara **Wasabi** dan penyimpanan cloud atau lokal lainnya, tanpa menggunakan command line.

Dengan RcloneView Anda dapat:

- Menjelajahi bucket Wasabi Anda seperti folder lokal  
- Drag & drop berkas antara **disk lokal ↔ Wasabi** atau **Wasabi ↔ cloud lainnya**  
- Menjalankan transfer sekali jalan atau menjadwalkan sinkronisasi berulang  

Jika Anda lebih suka melihatnya beraksi terlebih dahulu, Anda dapat menonton demo singkat (menyesuaikan ukuran otomatis, tanpa letterbox):
...
1. Kunjungi halaman unduhan: [https://rcloneview.com/src/download](https://rcloneview.com/src/download).  
...
RcloneView memperlakukan Wasabi sebagai backend yang **kompatibel dengan S3**. Anda membuat remote sekali dan kemudian menggunakannya kembali untuk menjelajahi, menyalin, sinkronisasi, dan pekerjaan terjadwal.
...
### 2.3 Mengonfigurasi Wasabi sebagai provider yang kompatibel dengan S3
...
2. Konfirmasi bahwa remote Wasabi Anda (misalnya `MyWasabi`) terdaftar dan dapat dijangkau.
...
Untuk detail lebih lanjut, lihat panduan umum yang kompatibel dengan S3:  
...
3. RcloneView membuka remote dalam sebuah tab tempat bucket muncul seperti folder tingkat atas.  
...
Untuk tips navigasi yang lebih umum, lihat:  
- [File Management with RcloneView](/howto/rcloneview-basic/browse-and-manage-remote-storage)

---

## 4. Mengelola berkas antara Disk Lokal dan Wasabi
...
Kemudian gunakan drag & drop, perbandingan folder, atau sinkronisasi tergantung alur kerja Anda.

---

### 4.1 Drag & drop antara Lokal dan Wasabi

Drag & drop adalah cara termudah untuk menyalin berkas.

1. Di panel kiri, buka **folder lokal** Anda (misalnya `D:\Media`).  
2. Di panel kanan, buka **bucket/folder Wasabi** Anda.  
3. Pilih berkas atau folder di sebelah kiri.  
4. Seret ke panel kanan dan lepaskan pada lokasi yang diinginkan.  
5. RcloneView memulai pekerjaan transfer; progres muncul di tab **Transfer**.

<img src="/support/images/en/tutorials/wasabi-drag-and-drop.png" alt="wasabi drag and drop" class="img-large img-center" />
Untuk pemilihan banyak berkas, tindakan klik kanan, dan lainnya, lihat:  
- [File Management with RcloneView](/howto/rcloneview-basic/browse-and-manage-remote-storage)

---

### 4.2 Membandingkan folder dan menyalin berkas yang berubah
...
4. RcloneView menandai:
   - Berkas hanya di kiri (hanya sumber)  
   - Berkas hanya di kanan (hanya tujuan)  
   - Berkas yang berubah (ukuran, timestamp, atau checksum berbeda)  
...
### 4.3 Sinkronisasi antara Disk Lokal dan Wasabi
...
- **Saved Sync Job** (jalankan kembali saat diperlukan)  
- **Scheduled Sync Job** (jalankan otomatis sesuai jadwal)  

> ⚠️ Sync memperbarui tujuan agar sesuai dengan sumber; berkas yang hanya ada di tujuan dapat dihapus. Tinjau pengaturan sinkronisasi dengan saksama sebelum menjalankannya.

#### 4.3.1 Instant Sync (sekali jalan)
...
Setelah menjalankan Sync, Anda dapat memantau progres transfer berkas secara real-time.
...
1. Di dialog Sync, pilih **Save to Jobs** alih-alih menjalankan langsung.  
2. Berikan nama yang deskriptif pada pekerjaan tersebut, seperti `SyncLocalToWasabi`.  
3. Selanjutnya, buka **Job Manager** dan jalankan pekerjaan tersebut secara manual kapan pun Anda memerlukan pencadangan terbaru.
...
Pada platform yang didukung (seperti Windows), RcloneView dapat menampilkan notifikasi sistem ketika pekerjaan selesai.

Referensi:  
- [Create Sync Jobs](/howto/rcloneview-basic/create-sync-jobs)  
- [Execute & Manage Jobs](/howto/rcloneview-basic/execute-manage-job)

#### 4.3.3 Menjadwalkan pencadangan otomatis Wasabi (Penjadwalan Pekerjaan)

Untuk sepenuhnya mengotomatiskan pencadangan ke Wasabi, jadwalkan sync job Anda.

> 📌 **Penjadwalan pekerjaan adalah fitur PLUS.** Anda memerlukan [lisensi PLUS](https://rcloneview.com/src/pricing.html) untuk mengaktifkan penjadwalan.

Buka **Job Manager** dari toolbar.    
1. Pilih sync job Wasabi Anda (misalnya `LocalToWasabi_DailyBackup`) dan klik **Edit Job**, atau buat pekerjaan baru dari pilihan Explorer Anda saat ini.  
...
5. Simpan pekerjaan dan biarkan RcloneView tetap berjalan; pekerjaan akan dijalankan secara otomatis.
...
Anda dapat mount bucket Wasabi sebagai **drive atau folder virtual** di sistem Anda dan kemudian menggunakan Windows Explorer (atau Finder di macOS) seperti biasa.
...
4. Setelah beberapa saat, drive atau folder baru muncul di OS Anda. Menjelajahi path tersebut membaca dan menulis data langsung dari Wasabi melalui RcloneView/rclone.
...
## 5. Ke mana mendapatkan bantuan

- Dokumentasi dan panduan cara penggunaan RcloneView: [https://rcloneview.com/support](https://rcloneview.com/support)  
- Portal Dokumentasi Wasabi: [https://docs.wasabi.com](https://docs.wasabi.com)  

Dengan menggabungkan penyimpanan objek Wasabi yang kompatibel dengan S3 dan manajemen pekerjaan visual RcloneView, Anda dapat membangun alur kerja pencadangan, sinkronisasi, dan migrasi yang andal tanpa harus mempelajari sintaks command-line rclone.
