---
description: 了解如何使用 RcloneView 搭配 Wasabi，在本機儲存空間與其他雲端之間瀏覽、備份、同步及遷移資料。
keywords:
  - rcloneview
  - wasabi
  - s3-compatible
...
# 如何在 RcloneView 中使用 Wasabi？

RcloneView 是一款桌面應用程式，為 **rclone** 提供視覺化的雙窗格檔案總管介面。它讓您無需使用命令列，即可在 **Wasabi** 與其他雲端或本機儲存空間之間複製、同步及遷移檔案。

透過 RcloneView，您可以：

- 像瀏覽本機資料夾一樣瀏覽您的 Wasabi 儲存桶（bucket）  
- 在 **本機磁碟 ↔ Wasabi** 或 **Wasabi ↔ 其他雲端** 之間拖放檔案  
- 執行一次性傳輸，或排程週期性的同步工作  

如果您想先看看實際操作效果，可以觀看簡短的示範影片（自動調整大小，無黑邊）：
...
RcloneView 支援 **Windows、macOS 和 Linux**。
...
## 2. 在 RcloneView 中新增 Wasabi 作為遠端

RcloneView 將 Wasabi 視為 **相容 S3** 的後端。您只需建立一次遠端，之後即可重複用於瀏覽、複製、同步及排程工作。

### 2.1 前置需求 — Wasabi 存取金鑰與端點

若要將 RcloneView 連接到 Wasabi，您需要：

- **存取金鑰（Access Key）** / **密鑰（Secret Key）**  
- **區域 / 端點網址（Region / Endpoint URL）**（例如區域 `ap-northeast-2`、端點 `s3.ap-northeast-2.wasabisys.com`）  

請參閱 Wasabi 官方文件以建立存取金鑰並找到您的端點：

- Wasabi 文件：[https://docs.wasabi.com/docs](https://docs.wasabi.com/docs)  
- 範例：「[Creating a New Access Key](https://docs.wasabi.com/docs/creating-a-new-access-key)」或「[Creating a Bucket](https://docs.wasabi.com/docs/creating-a-bucket)」（於 Wasabi 文件入口網站內搜尋）。

取得 **存取金鑰**、**密鑰** 及 **端點** 後，請返回 RcloneView。

### 2.2 開啟「新增遠端」精靈

1. 啟動 **RcloneView**。  
2. 從頂端選單，點擊 **`Remote` → `+ New Remote`**。  
   - 或於檔案總管窗格點擊 **`+`** 分頁並選擇 **`New Remote`**。
...
### 2.3 將 Wasabi 設定為相容 S3 的提供者

1. 在 **New Remote** 對話框中，搜尋 `Wasabi`。  
2. 選擇 **Wasabi** 供應商圖示。  
3. 設定連線資訊：
   - **Remote name（遠端名稱）**：輸入清楚易懂的名稱，例如 `MyWasabi`。  
   - **Access Key ID**：貼上您的 Wasabi 存取金鑰。  
   - **Secret Access Key**：貼上您的 Wasabi 密鑰。  
   - **Endpoint**：輸入 Wasabi 的 S3 端點（例如 `s3.ap-northeast-2.wasabisys.com`）。  
4. 點擊 **Add Remote**。
...
3. RcloneView 會在新分頁中開啟該遠端，儲存桶會顯示為最上層資料夾。  
4. 雙擊儲存桶即可瀏覽其內容。
...
本節將說明如何使用 RcloneView 在您的本機電腦與 Wasabi 之間實際移動資料。
...
接著依照您的工作流程，使用拖放、資料夾比較或同步工作。

---

### 4.1 在本機與 Wasabi 之間拖放檔案
...
若您想在複製前先查看 **差異**，可使用 **Compare（比較）** 功能。

常見情境：比較本機備份資料夾與 Wasabi 備份資料夾。

1. 在左側窗格中，開啟 **來源資料夾**（例如本機或其他雲端）。  
2. 在右側窗格中，開啟 **Wasabi 目的地資料夾**。  
3. 點擊頂端 **Home** 選單中的 **`Compare`**。  
4. RcloneView 會標記：
   - 僅存在於左側的檔案（僅來源端有）  
   - 僅存在於右側的檔案（僅目的地端有）  
   - 有變更的檔案（大小、時間戳記或校驗碼不同）  
5. 選取您要移動的項目，並點擊 **Copy →**（或反方向的 **← Copy**）。
...
若需要可重複執行的備份或鏡像，請使用 **Sync（同步）**。同步會讓目的地與來源保持一致。

常見的三種模式：

- **Instant Sync（即時同步）**（立即執行一次）  
- **Saved Sync Job（已儲存的同步工作）**（需要時可重新執行）  
- **Scheduled Sync Job（排程同步工作）**（依排程自動執行）  

> ⚠️ 同步會將目的地更新為與來源一致；僅存在於目的地的檔案可能會被刪除。執行前請務必仔細檢查同步設定。

#### 4.3.1 即時同步（一次性）

1. 在左側窗格開啟 **來源資料夾**（例如本機資料夾）。  
2. 在右側窗格開啟 **Wasabi 目的地資料夾**。  
3. 點擊 **Home** 選單中的 **`Sync`**。  
...
1. 於 **Configure Storage** 確認來源與目的地。  
...
若您將定期執行相同的本機 → Wasabi 備份：  
請如上所述設定同步（左側為來源，右側為 Wasabi 目的地）。

1. 在 Sync 對話框中，選擇 **Save to Jobs**，而非立即執行。  
2. 為工作命名一個容易辨識的名稱，例如 `SyncLocalToWasabi`。  
3. 之後，開啟 **Job Manager** 並在需要更新備份時手動執行該工作。
...
在支援的平台上（例如 Windows），工作完成時 RcloneView 可顯示系統通知。
...
若要完全自動化備份至 Wasabi，請為您的同步工作設定排程。

> 📌 **工作排程屬於 PLUS 功能。** 您需要 [PLUS license](https://rcloneview.com/src/pricing.html) 才能啟用排程。

從工具列開啟 **Job Manager**。  
1. 選取您的 Wasabi 同步工作（例如 `LocalToWasabi_DailyBackup`）並點擊 **Edit Job**，或依目前檔案總管的選取項目建立新工作。  
2. 前往 **Step 4: Scheduling**。  
3. 啟用 **Run whenever time units ~** 並設定排程（例如每天 01:00）。  
4. 使用 **Simulate** 預覽即將執行的時間。  
5. 儲存工作並讓 RcloneView 保持執行；該工作將會自動執行。
...
### 4.4 使用「掛載」在 Windows 檔案總管中操作 Wasabi

您可以將 Wasabi 儲存桶掛載為系統上的 **虛擬磁碟機或資料夾**，然後照常使用 Windows 檔案總管（或 macOS 的 Finder）操作。

典型流程：

確認您的 Wasabi 遠端已設定完成且運作正常。  
1. 選擇您要掛載的 Wasabi 資料夾。  
2. 點擊 RcloneView 檔案總管右上角的 **Mount** 圖示。  
3. 點擊 **Save and mount** 按鈕開始掛載。  
4. 稍待片刻後，您的作業系統中會出現新的磁碟機或資料夾。瀏覽該路徑即可透過 RcloneView/rclone 直接讀寫 Wasabi 的資料。
...
透過結合 Wasabi 相容 S3 的物件儲存與 RcloneView 的視覺化工作管理，您無需學習 rclone 命令列語法，即可建立可靠的備份、同步及遷移工作流程。
