---
description: "Découvrez comment utiliser RcloneView avec Wasabi pour parcourir, sauvegarder, synchroniser et migrer des données entre un stockage local et d'autres clouds."
keywords:
  - rcloneview
  - wasabi
  - s3-compatible
...
# Comment utiliser RcloneView avec Wasabi ?

RcloneView est une application de bureau qui offre un explorateur visuel à deux volets pour **rclone**. Elle vous permet de copier, synchroniser et migrer des fichiers entre **Wasabi** et d'autres stockages cloud ou locaux, sans utiliser la ligne de commande.
...
Si vous préférez d'abord voir cela en action, vous pouvez regarder une courte démo (redimensionnement automatique, sans bandes noires) :
...
> Pour en savoir plus sur RcloneView, visitez le site officiel : [https://rcloneview.com](https://rcloneview.com)  
...
2. Choisissez l'installeur pour votre système d'exploitation (Windows, macOS ou Linux).  
...
RcloneView traite Wasabi comme un backend **compatible S3**. Vous créez un remote une seule fois, puis vous le réutilisez pour la navigation, la copie, la synchronisation et les tâches planifiées.
...
1. Lancez **RcloneView**.  
2. Dans le menu du haut, cliquez sur **`Remote` → `+ New Remote`**.  
   - Ou cliquez sur l'onglet **`+`** dans le volet Explorer et choisissez **`New Remote`**.
...
3. Configurez la connexion :
   - **Remote name** : saisissez un nom clair, par exemple `MyWasabi`.  
   - **Access Key ID** : collez votre access key Wasabi.  
   - **Secret Access Key** : collez votre secret key Wasabi.  
...
### 2.4 Vérifier le remote Wasabi

1. Ouvrez **`Remote → Remote Manager`**.  
2. Confirmez que votre remote Wasabi (par exemple `MyWasabi`) est répertorié et accessible.
...
Une fois le remote créé, vous pouvez parcourir les buckets et les objets dans l'Explorer de RcloneView.

1. Dans le volet Explorer, cliquez sur l'onglet **`+`**.  
2. Dans la liste « Open Remote », choisissez votre remote **Wasabi** (par exemple `MyWasabi`).  
3. RcloneView ouvre le remote dans un onglet où les buckets apparaissent comme des dossiers de premier niveau.  
...
Pour d'autres conseils généraux de navigation, consultez :  
...
Cette section présente des moyens pratiques de déplacer des données entre votre ordinateur local et Wasabi avec RcloneView.

Vous pouvez ouvrir :

- **Volet gauche** : disque local (par exemple `C:\` ou un dossier spécifique)  
- **Volet droit** : votre bucket remote Wasabi  
...
1. Dans le volet gauche, ouvrez votre **dossier local** (par exemple `D:\Media`).  
2. Dans le volet droit, ouvrez votre **bucket/dossier Wasabi**.  
3. Sélectionnez des fichiers ou dossiers à gauche.  
4. Faites-les glisser vers le volet droit et déposez-les à l'emplacement souhaité.  
...
Si vous voulez voir les **différences** avant de copier, utilisez la fonction **Compare**.

Cas d'usage typique : comparer un dossier de sauvegarde local avec un dossier de sauvegarde Wasabi.

1. Dans le volet gauche, ouvrez le **dossier source** (par exemple local ou un autre cloud).  
2. Dans le volet droit, ouvrez le **dossier de destination Wasabi**.  
3. Cliquez sur **`Compare`** dans le menu supérieur **Home**.  
4. RcloneView marque :
   ...
   - Les fichiers modifiés (taille, horodatage ou checksum différents)  
5. Sélectionnez les éléments que vous souhaitez déplacer et cliquez sur **Copy →** (ou **← Copy** pour la direction inverse).
...
- **Saved Sync Job** (ré-exécutable au besoin)  
...
1. Ouvrez le **dossier source** dans le volet gauche (par exemple, un dossier local).  
2. Ouvrez le **dossier de destination Wasabi** dans le volet droit.  
...
3. Lancez d'abord un **Dry Run** si vous souhaitez prévisualiser les changements.  
...
Après avoir lancé Sync, vous pouvez suivre la progression du transfert de fichiers en temps réel.
...
Si vous devez exécuter régulièrement la même sauvegarde local → Wasabi :  
...
2. Donnez à la tâche un nom descriptif comme `SyncLocalToWasabi`.  
...
#### 4.3.3 Planifier des sauvegardes Wasabi automatiques (Job Scheduling)
...
1. Sélectionnez votre tâche de synchronisation Wasabi (par exemple `LocalToWasabi_DailyBackup`) et cliquez sur **Edit Job**, ou créez une nouvelle tâche à partir de votre sélection actuelle dans l'Explorer.  
2. Allez à **Step 4: Scheduling**.  
...
Vous pouvez monter un bucket Wasabi comme **lecteur ou dossier virtuel** sur votre système, puis utiliser l'explorateur Windows (ou Finder sur macOS) comme d'habitude.

Flux typique :

Assurez-vous que votre remote Wasabi est configuré et fonctionnel.  
1. Choisissez le dossier Wasabi que vous souhaitez monter.  
2. Cliquez sur l'icône **Mount** dans le coin supérieur droit de l'Explorer de RcloneView.  
3. Cliquez sur le bouton **Save and mount** pour démarrer le montage.  
4. Après quelques instants, un nouveau lecteur ou dossier apparaît dans votre système d'exploitation. Parcourir ce chemin lit et écrit les données directement depuis Wasabi via RcloneView/rclone.
...
- Documentation et guides pratiques de RcloneView : [https://rcloneview.com/support](https://rcloneview.com/support)  
- Portail de documentation Wasabi : [https://docs.wasabi.com](https://docs.wasabi.com)  

En combinant le stockage objet compatible S3 de Wasabi avec la gestion visuelle des tâches de RcloneView, vous pouvez créer des workflows fiables de sauvegarde, de synchronisation et de migration sans avoir à apprendre la syntaxe en ligne de commande de rclone.
