---
sidebar_position: 2
description: RcloneView와 Wasabi를 사용해 로컬 스토리지와 다른 클라우드 간에 데이터를 탐색, 백업, 동기화, 마이그레이션하는 방법을 알아보세요.
keywords:
  - rcloneview
  - wasabi
  - s3-compatible
...
- **로컬 디스크 ↔ Wasabi** 또는 **Wasabi ↔ 다른 클라우드** 간 파일 드래그 앤 드롭  
- 일회성 전송 실행 또는 반복 동기화 작업 예약  

먼저 실제 동작을 보고 싶다면 짧은 데모 영상을 시청할 수 있습니다:
...
RcloneView는 **Windows, macOS, Linux**에서 사용할 수 있습니다.

1. 다운로드 페이지로 이동합니다: [https://rcloneview.com/src/download](https://rcloneview.com/src/download).  
2. 사용 중인 OS(Windows, macOS 또는 Linux)에 맞는 설치 프로그램을 선택합니다.  
3. **RcloneView**를 설치하고 실행합니다.

---

## 2. RcloneView에 Wasabi를 리모트로 추가하기

RcloneView는 Wasabi를 **S3 호환** 백엔드로 취급합니다. 리모트를 한 번 생성하면 탐색, 복사, 동기화, 예약 작업에 재사용할 수 있습니다.

### 2.1 사전 준비 – Wasabi 액세스 키 및 엔드포인트

RcloneView를 Wasabi에 연결하려면 다음이 필요합니다:

- **액세스 키(Access Key)** / **시크릿 키(Secret Key)**  
- **리전 / 엔드포인트 URL** (예: 리전 `ap-northeast-2` 및 엔드포인트 `s3.ap-northeast-2.wasabisys.com`)  

액세스 키를 생성하고 엔드포인트를 확인하려면 Wasabi의 공식 문서를 참고하세요:
...
**액세스 키**, **시크릿 키**, **엔드포인트**를 확보했다면 RcloneView로 돌아옵니다.

### 2.2 "New Remote" 마법사 열기

1. **RcloneView**를 실행합니다.  
2. 상단 메뉴에서 **`Remote` → `+ New Remote`**를 클릭합니다.  
   - 또는 탐색기(Explorer) 패널의 **`+`** 탭을 클릭하고 **`New Remote`**를 선택합니다.
...
### 2.3 Wasabi를 S3 호환 공급자로 설정하기

1. **New Remote** 대화상자에서 `Wasabi`를 검색합니다.  
2. **Wasabi** 공급자 타일을 선택합니다.  
3. 연결을 구성합니다:
   - **리모트 이름**: `MyWasabi`처럼 알아보기 쉬운 이름을 입력합니다.  
   - **Access Key ID**: Wasabi 액세스 키를 붙여넣습니다.  
   - **Secret Access Key**: Wasabi 시크릿 키를 붙여넣습니다.  
...
2. Wasabi 리모트(예: `MyWasabi`)가 목록에 표시되고 연결 가능한지 확인합니다.
...
리모트가 생성되면 RcloneView의 탐색기(Explorer)에서 버킷과 객체를 탐색할 수 있습니다.

1. 탐색기 패널에서 **`+`** 탭을 클릭합니다.  
2. "Open Remote" 목록에서 **Wasabi** 리모트(예: `MyWasabi`)를 선택합니다.  
3. RcloneView는 버킷이 최상위 폴더처럼 표시되는 탭에서 리모트를 엽니다.  
4. 버킷을 더블클릭하여 내용을 탐색합니다.
...
1. 왼쪽 패널에서 **로컬 폴더**(예: `D:\Media`)를 엽니다.  
2. 오른쪽 패널에서 **Wasabi 버킷/폴더**를 엽니다.  
3. 왼쪽에서 파일 또는 폴더를 선택합니다.  
4. 오른쪽 패널로 드래그하여 원하는 위치에 드롭합니다.  
...
다중 선택, 우클릭 작업 등에 대한 자세한 내용은 다음을 참고하세요:  
- [File Management with RcloneView](/howto/rcloneview-basic/browse-and-manage-remote-storage)

---

### 4.2 폴더 비교 및 변경된 파일 복사

복사하기 전에 **차이점**을 확인하고 싶다면 **Compare** 기능을 사용하세요.

일반적인 사용 사례: 로컬 백업 폴더와 Wasabi 백업 폴더를 비교합니다.

1. 왼쪽 패널에서 **원본 폴더**(예: 로컬 또는 다른 클라우드)를 엽니다.  
2. 오른쪽 패널에서 **Wasabi 대상 폴더**를 엽니다.  
3. 상단 **Home** 메뉴에서 **`Compare`**를 클릭합니다.  
4. RcloneView는 다음을 표시합니다:
   - 왼쪽에만 있는 파일(원본에만 존재)  
   - 오른쪽에만 있는 파일(대상에만 존재)  
   - 변경된 파일(크기, 타임스탬프 또는 체크섬이 다름)  
5. 이동할 항목을 선택하고 **Copy →**(반대 방향은 **← Copy**)를 클릭합니다.
...
- **즉시 동기화**(Instant Sync, 즉시 한 번 실행)  
- **저장된 동기화 작업**(Saved Sync Job, 필요할 때 재실행)  
- **예약된 동기화 작업**(Scheduled Sync Job, 일정에 따라 자동 실행)  

> ⚠️ 동기화는 원본과 일치하도록 대상을 업데이트하며, 대상에만 존재하는 파일은 삭제될 수 있습니다. 실행하기 전에 동기화 설정을 신중히 확인하세요.

#### 4.3.1 즉시 동기화(일회성)

1. 왼쪽 패널에서 **원본 폴더**(예: 로컬 폴더)를 엽니다.  
2. 오른쪽 패널에서 **대상 Wasabi 폴더**를 엽니다.  
...
2. 필요에 따라 **Advanced Settings** 또는 **Filtering Settings**을 조정합니다.  
3. 변경 사항을 미리 보려면 먼저 **Dry Run**을 실행합니다.  
...
동기화 실행 후, 파일 전송 진행 상황을 실시간으로 모니터링할 수 있습니다.
...
#### 4.3.2 재사용을 위해 동기화 작업 저장하기

동일한 로컬 → Wasabi 백업을 정기적으로 실행할 예정이라면:  
위와 같이 동기화를 구성합니다(왼쪽에 원본, 오른쪽에 Wasabi 대상).    

1. Sync 대화상자에서 즉시 실행하는 대신 **Save to Jobs**를 선택합니다.  
2. `SyncLocalToWasabi`처럼 알아보기 쉬운 이름을 작업에 지정합니다.  
3. 이후 업데이트된 백업이 필요할 때마다 **Job Manager**를 열어 해당 작업을 수동으로 실행합니다.
...
2. **4단계: Scheduling**으로 이동합니다.  
3. **Run whenever time units ~**를 활성화하고 일정을 설정합니다(예: 매일 01:00).  
4. **Simulate**를 사용해 다가오는 실행 시간을 미리 확인합니다.  
5. 작업을 저장하고 RcloneView를 계속 실행 상태로 유지하면 작업이 자동으로 실행됩니다.
...
### 4.4 Windows 탐색기에서 Wasabi를 사용하기 위한 마운트 활용

Wasabi 버킷을 시스템의 **가상 드라이브 또는 폴더**로 마운트한 후 평소처럼 Windows 탐색기(또는 macOS의 Finder)를 사용할 수 있습니다.

일반적인 흐름:

Wasabi 리모트가 구성되어 정상 작동하는지 확인하세요.  
1. 마운트하려는 Wasabi 폴더를 선택합니다.  
2. RcloneView 탐색기(Explorer) 오른쪽 상단의 **Mount** 아이콘을 클릭합니다.  
3. **Save and mount** 버튼을 클릭하여 마운트를 시작합니다.  
4. 잠시 후 OS에 새 드라이브 또는 폴더가 나타납니다. 해당 경로를 탐색하면 RcloneView/rclone을 통해 Wasabi에서 직접 데이터를 읽고 씁니다.
...
Wasabi의 S3 호환 오브젝트 스토리지와 RcloneView의 시각적 작업 관리를 결합하면, rclone 명령줄 문법을 배우지 않고도 안정적인 백업, 동기화, 마이그레이션 워크플로를 구축할 수 있습니다.
