...
4. 글 발행 시 8로케일 번역을 같은 PR에 포함 — **구현됨**: Fact-checker STEP 4.7 이 검증 통과 글을 **인라인(자기 Read/Write)** 으로 번역하고 2장 ③ 검증을 돈다. ⚠️ **루틴은 Workflow 도구/서브에이전트를 못 쓴다** — 무인 루틴에서 Workflow 도구가 승인 프롬프트에 걸려 멈추기 때문(2026-07-21 실측). `scripts/blog-i18n-batch.workflow.js`(병렬 Sonnet)는 **로컬 소급 번역 전용**으로만 유지. STEP 5 빌드는 `npm run build`(prebuild 이미지검사 + postbuild 프룬)로 검증만 한다 — www 반영은 `.github/workflows/deploy-www.yml` 이 main 머지 후 자동으로 처리한다(2026-07-28). STEP 6-B 는 영어 원문 + `i18n/*/docusaurus-plugin-content-blog/${DATE}-*.md`(8로케일 번역)을 verified 브랜치에 함께 커밋 — support 가 번역 원본이므로 소스에 남긴다. **3단계 계획 전부 완료.**
